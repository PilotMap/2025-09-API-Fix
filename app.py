...
# Initialize LED strip for web interface
try:
    strip = LedStrip(config.LED_COUNT)
    logger.info(f"LED strip initialized with {strip.number} LEDs")
except Exception as e:
    logger.warning(f"LED strip initialization failed: {e}")
    # Create a dummy strip object for web interface
    class DummyStrip:
        def __init__(self):
            self.number = config.LED_COUNT
    strip = DummyStrip()
    ...
    while tmp_ap >= 0:
        ...
        while internet_test:  # check internet availability and retry if necessary. If house power outage, map may boot quicker than router.
            try:
                ...
                if internet_tries <= 0:
                    internet_test = False
                    logger.warning("No Internet - Skipping airport info loading")
                    return
                pass
...
def setup():
    ...
    # Try to get airport info, but don't crash if it fails
    try:
        get_apinfo()  # decode airports to get city and state of each airport
        logger.info("Airport info loaded successfully")
    except Exception as e:
        logger.warning(f"Failed to load airport info: {e}")
        logger.info("Continuing without airport info...")
    
    try:
        get_led_map_info() # get airport location in lat lon and flight category
        logger.info("LED map info loaded successfully")
    except Exception as e:
        logger.warning(f"Failed to load LED map info: {e}")
        logger.info("Continuing without LED map info...")
