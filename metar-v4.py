...
while (outerloop):
    ...
    if metar_taf_mos != 2 and metar_taf_mos != 3:
          ...
          if(chunk >= 300):
             ...
             while True: #check internet availability and retry if necessary. If house power outage, map may boot quicker than router.
              ...
              try:
                ...
                logger.info(f"Raw response has {len(r)} lines")
                logger.info(f"First 5 lines: {r[:5]}")
                logger.info(f"Last 5 lines: {r[-5:]}")
                
                # 2025 API: Parse the full XML response directly
                xml_content = '\n'.join(r)
                ...
                logger.info(f"Root has {len(root)} children")
        ...
        # XML parsing is now handled in the individual API calls above
