...
print(f"Initializing LED strip with {LED_COUNT} LEDs...")
try:
    strip = LedStrip(LED_COUNT)
    print(f"✅ LED strip initialized successfully with {strip.number} LEDs")
...
while (outerloop):
    ...
    if metar_taf_mos != 2 and metar_taf_mos != 3:
        # Use enhanced API client for robust batching
        client = create_api_client()
        try:
            if metar_taf_mos == 1:  # METAR
                root = client.get_metar_data(airports, hours=metar_age)
            elif metar_taf_mos == 0:  # TAF
                root = client.get_taf_data(airports, hours=metar_age)
            logger.info('Weather data retrieved successfully')
        except Exception as e:
            logger.error(f'Failed to retrieve weather data: {e}')
            # Continue with empty root to prevent script crash
            root = ET.fromstring('<x><data num_results="0"></data></x>')
...
#                            logger.debug(cld_base_ft_agl) #debug
        ...
        # Debug: Check if we have METAR elements
        metar_count = len(list(root.iter('METAR')))
        logger.info(f"Found {metar_count} METAR elements in XML response")
        
        # Debug: Check what elements we do have
        logger.info(f"Root element: {root.tag}")
        for child in root:
            logger.info(f"Child element: {child.tag}")
            if child.tag == 'data':
                logger.info(f"Data element has {len(list(child))} children")
                for data_child in child:
                    logger.info(f"Data child: {data_child.tag}")
        
        # 2025 API: Look for METAR elements in the data section
        data_elem = root.find('data')
        if data_elem is not None:
            metar_elements = data_elem.findall('METAR')
            logger.info(f"Found {len(metar_elements)} METAR elements in data section")
        else:
            metar_elements = []
            logger.warning("No data element found in XML response")
        
        # Track processed airports to avoid duplicates
        processed_airports = set()
        
        for metar in metar_elements:
            stationId = metar.find('station_id').text
            
            # Skip if we've already processed this airport (use most recent METAR)
            if stationId in processed_airports:
                logger.debug(f"Skipping duplicate METAR for {stationId}")
                continue
                
            processed_airports.add(stationId)
            logger.info(f"Processing METAR for station: {stationId}")
            ...
            if api_flight_category_available and prefer_api_flight_category and not force_fallback_calculation:
                ...
                logger.info(f"{stationId}: Final flight category calculated as: {flightcategory}")
        ...
        logger.info(f"Station dictionary contains {len(stationiddict)} airports: {list(stationiddict.keys())[:10]}...")
        logger.info(f"Airports list contains {len(airports)} airports: {airports[:10]}...")
    ...
    while time.time() < timeout_start + (update_interval * 60): #take 'update_interval' which is in minutes and turn into seconds
        ...
        for cycle_num in cycles: #cycle through the strip 6 times, setting the color then displaying to create various effects.
            ...
            for airportcode in airports:
                ...
                color = color_nowx  # Default to "no weather" color

                # Handle legend entries and regular airports differently
                if airportcode == "LGND":
                    flightcategory = "NONE"  # Legend entries don't have weather data
                    airportwinds = 0
                    airportwx_long = "NONE"
                    airportwx = "NONE"
                else:
        ...
        #          if(airportwinds != ''):
              #      print(airportcode, 'airportwinds',type(airportwinds),":".join("{:02x}".format(ord(c)) for c in airportwinds), 'max_wind_speed', max_wind_speed,'cycle_num',cycle_num)
                ...
                if i == homeport_pin and homeport: #if this is the home airport, don't dim out the brightness
                    # xcolor is already a list from rgbtogrb, convert to Color
                    xcolor = Color(int(xcolor[0]), int(xcolor[1]), int(xcolor[2]))
